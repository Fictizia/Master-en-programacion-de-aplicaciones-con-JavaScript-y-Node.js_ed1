var app = {
  ...
  renderView:{
    ...
    fillCountryDetails:function (json) {
      if (json) {
        ...
        // @Ulises: Simplificar esto atacnado a div.country-info con toda la estructura
        ...
        flag.src = json.flag;
        //@Ulises: OJO! 5 modificaciones del DOM...
    ...
    maps:{
      // @Ulises: Estilos avanzados: https://snazzymaps.com/
      initCountryMap:function (address) {
        if (address) {
          ...
          var map = new google.maps.Map(document.getElementById('map'), {
          //Ulises: GEO-Posicionar al usuario con el zoom: 17 y dejarle un marker
    ...
    // @ULISES: Posibilidad de incluir toggle??
  ...
  tools:{
    // @Ulises: Simplificar los parametros en (details[obj], callback[fn])
    // @Ulises: Validacion y parametros por defecto
    // @ulises: Validar el json con un try-catch
...
// @Ulises: Extender la idea a todo el codigo...
(function () {
  var element = document.querySelector('.loader');
  app.renderView.showHideElement(true,element);
